...
// helper to load a row of a matrix given a line from the input file and its
// matrix row index
int loadInnerMatrix(char *line, int *indexArray, size_t rowIndex,
                    size_t columnSize) {
...
int dotProduct(Matrix *matrix1, Matrix *matrix2, Matrix *product) {
  ...
  // first matrix input is parsed by row first, second matrix input is parsed by
  // column first
  for (size_t counter1 = 0; counter1 < (*matrix1).R; counter1++) {
    for (size_t counter2 = 0; counter2 < (*matrix2).C; counter2++) {
      ...
      for (size_t counter3 = 0; counter3 < (*matrix1).C; counter3++) {
        savedOutput += matrix1->index[(counter1 * matrix1->C) + counter3] *
                       matrix2->index[(counter3 * matrix2->C) + counter2];
      }
